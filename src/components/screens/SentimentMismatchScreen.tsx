...
import { AlertTriangle, Zap, Frown, X, ChevronRight, HelpCircle } from "lucide-react";
import {
  Drawer,
  DrawerContent,
  DrawerHeader,
...
const SentimentMismatchScreen: React.FC = () => {
  ...
  return (
    <ScreenShell>
      ...
      >
        Your experience doesn't match what our systems are showing. We understand how frustrating that can be, and we want to help.
      </motion.p>
      ...
      >
        <div className="grid grid-cols-2 gap-3">
          <div className="p-4 rounded-xl bg-success/5 border border-success/20 text-center">
            ...
            <p className="text-sm font-bold text-success">Supercharged WiFi</p>
          </div>
          <div className="p-4 rounded-xl bg-destructive/5 border border-destructive/20 text-center">
            ...
            <p className="text-sm font-bold text-destructive">Negative</p>
          </div>
        </div>
      </motion.div>
      ...
      >
        ...
        <ActionButton variant="outline" onClick={() => setCurrentState("mismatch_actions")} icon={<ChevronRight className="w-5 h-5" />}>
          What should I do?
        </ActionButton>
      </motion.div>
        ...
        <DrawerContent>
          ...
          <div className="px-6 pb-4 space-y-4">
            <p className="text-sm text-muted-foreground leading-relaxed">
              Our diagnostic systems monitor your connection remotely, but they can't always capture the full picture of what happens inside your home.
            </p>
            <div className="space-y-3">
              <h3 className="text-sm font-semibold text-foreground">Why this can happen:</h3>
              <ul className="space-y-2">
                {[
                  "WiFi interference from nearby devices or neighbours",
                  "Physical obstacles (walls, floors) weakening the signal",
                  "Issues with specific devices that our systems can't detect",
                  "Temporary slowdowns that resolve before our checks run",
                ].map((reason, i) => (
            ...
            <div className="p-3 rounded-lg bg-muted/50">
              <p className="text-sm text-muted-foreground">
                <span className="font-medium text-foreground">What we suggest:</span> Try the troubleshooting steps on the next screen, or contact our support team for personalised help.
              </p>
            </div>
          </div>
