...
const ScanDoneScreen: React.FC = () => {
  ...
  return (
    <ScreenShell>
      ...
      <motion.p initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3, duration: 0.5 }} className="text-muted-foreground mb-6 text-sm">
        {t("scanDone.subtitle")}
      </motion.p>
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.35, duration: 0.5 }} className="w-full mb-8">
        <div className="flex items-center justify-between mb-3">
          {steps.map((step, i) => (
            <React.Fragment key={step}>
              <div className="flex flex-col items-center gap-1.5">
                <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ delay: 0.4 + i * 0.1, duration: 0.4, ease: [0.16, 1, 0.3, 1] }} className="w-10 h-10 rounded-full bg-success/15 flex items-center justify-center">
                  <CheckCircle2 className="w-5 h-5 text-success" />
                </motion.div>
                <span className="text-xs font-medium text-muted-foreground">{step}</span>
              </div>
              {i < steps.length - 1 && (
                <motion.div initial={{ scaleX: 0 }} animate={{ scaleX: 1 }} transition={{ delay: 0.45 + i * 0.1, duration: 0.3 }} className="flex-1 h-0.5 bg-success/30 mx-2 mb-5 origin-left" />
              )}
            </React.Fragment>
          ))}
        </div>
      </motion.div>
      <ActionButton onClick={handleSeeResult} icon={<ChevronRight className="w-5 h-5" />}>{t("scanDone.seeResult")}</ActionButton>
    </ScreenShell>
  );
};
