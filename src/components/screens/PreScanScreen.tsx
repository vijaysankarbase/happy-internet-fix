...
const checks = [
  { icon: <Router className="w-5 h-5 text-primary" />, label: "Modem status" },
  { icon: <Globe className="w-5 h-5 text-primary" />, label: "Network status" },
  { icon: <Wifi className="w-5 h-5 text-primary" />, label: "Connection quality" },
];
...
const PreScanScreen: React.FC = () => {
  ...
  return (
    <ScreenShell>
      ...
      >
        The scan takes a few seconds and will analyse three key areas of your connection.
      </motion.p>
      ...
      <div className="w-full flex flex-col gap-3">
        <ActionButton onClick={() => setCurrentState("scanning")} icon={<Scan className="w-5 h-5" />}>
          Start new scan
        </ActionButton>
        <ActionButton variant="outline" onClick={() => setCurrentState("retest_warning")} icon={<RotateCcw className="w-5 h-5" />}>
          Retest after a fix
        </ActionButton>
      </div>
    </ScreenShell>
