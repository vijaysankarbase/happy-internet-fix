...
const EntryScreen: React.FC = () => {
  ...
  return (
    <motion.div
      ...
      className="px-6 py-10 max-w-md mx-auto flex flex-col items-center justify-center min-h-[70vh]"
    >
      <motion.div
        initial={{ opacity: 0, scale: 0.92 }}
        animate={{ opacity: 1, scale: 1 }}
        transition={{ delay: 0.1, duration: 0.6, ease: [0.16, 1, 0.3, 1] }}
        className="w-full bg-card rounded-2xl border border-border shadow-lg overflow-hidden"
      >
        {/* Header row */}
        <div className="flex items-center gap-3 px-5 pt-5 pb-3">
          ...
          <div className="flex-1 min-w-0">
            <p className="text-xs font-semibold text-muted-foreground tracking-wide">My Internet Scan</p>
          </div>
          <button
